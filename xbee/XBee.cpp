...
bool XBee::available()
{
    if(serialxbee.isOpen()) return true;
    else return false;
}
...
RemoteAtCommandRequest::RemoteAtCommandRequest() : AtCommandRequest(nullptr,nullptr, 0) {
	_remoteAddress16 = 0;
	_applyChanges = false;
...
RemoteAtCommandRequest::RemoteAtCommandRequest(uint16_t remoteAddress16, uint8_t *command) : AtCommandRequest(command, nullptr, 0) {
	_remoteAddress64 = broadcastAddress64;
	_remoteAddress16 = remoteAddress16;
	_applyChanges = false;
	setApiId(REMOTE_AT_REQUEST);
...
RemoteAtCommandRequest::RemoteAtCommandRequest(XBeeAddress64 &remoteAddress64, uint8_t *command) : AtCommandRequest(command, nullptr, 0) {
	_remoteAddress64 = remoteAddress64;
	_remoteAddress16 = ZB_BROADCAST_ADDRESS;
	_applyChanges = false;
	setApiId(REMOTE_AT_REQUEST);
...
void XBee::flush(uint8_t dir)
{
    if(dir==1)      serialxbee.flush(input);
    else if(dir==2) serialxbee.flush(output);
    else if(dir==3) serialxbee.flush(bothDirection);
}
